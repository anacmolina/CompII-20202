...
Tetris::Tetris()
{
  std::cout << "Objeto de juego" << std::endl;

  start = 0; // Variable para empezar el juego                                                        
  value = 0;  // Contador de puntaje                                                                   
  screen[M][N] = {0};  // Ocupacion de la cuadricula (Copia)                                           
}
...
void Tetris::create_window()
{
  ...
  while (window.isOpen())  // Ventana abierta                                                            
      ...
      while (window.pollEvent(e))
	{
      ...
      {
        no_play(); // Mensaje para empezar a jugar                                                
      }
    else
      {
        int m;
...
if (!check()) // Si se pasa                                                                  
        ...
        draw_figure(color); // Dibuja la figura                                                 
        ...
        fixed_figure(color); // Fijar las figuras                                               
...
sf::Color Tetris::random_color(int n)
{
...
sf::RectangleShape Tetris::squares(int s, sf::Color color)
{
...
// Dibujar las figuras                                                                                   
void Tetris::draw_figure(sf::Color c) // Se pasa por referencia la ventana a dibujar   
{
...
void Tetris::traslation(int dx, int dy) // Recibe la variacion en x y y                                          
{
  if(dx!=0)
    {
...
void Tetris::rotation(int rotate)
{
  if (rotate == 1)
  {
...
void Tetris::fixed_figure(sf::Color color)
{
...
bool Tetris::top()
{
  for (int i = 0; i < N; i++)
    {
      if (screen[0][i] != 0) // Se recorre la segunda fila para determina si alguna celda esta ocupada   
          return 0;
    }
